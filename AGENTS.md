...
cd my-medusa-store/apps/backend && <pm> run lint       # medusa lint
cd my-medusa-store/apps/storefront && <pm> run lint    # next lint
```
...
cd my-medusa-store/apps/backend && <pm> run test:unit                      # **/src/**/__tests__/**/*.unit.spec.ts
cd my-medusa-store/apps/backend && <pm> run test:integration:modules       # **/src/modules/*/__tests__/**
cd my-medusa-store/apps/backend && <pm> run test:integration:http          # **/integration-tests/http/*.spec.ts
```

Single test — pass a path/pattern through to Jest, keeping `TEST_TYPE`:

```bash
cd my-medusa-store/apps/backend && <pm> run test:unit -- src/modules/foo/__tests__/service.unit.spec.ts
cd my-medusa-store/apps/backend && <pm> run test:unit -- -t "returns the cart"
```
...
- `building-admin-dashboard-customizations` — anything under `my-medusa-store/apps/backend/src/admin`
- `building-storefronts` — anything under `my-medusa-store/apps/storefront`
...
- Running storefront commands without checking that `my-medusa-store/apps/storefront/` exists.
- Assuming a package manager instead of detecting it, or running a command that creates a second lockfile.
- Installing a dependency at the root instead of inside the app that needs it (`cd my-medusa-store/apps/backend && <pm> add <pkg>`).
...
- `my-medusa-store/apps/backend/.medusa/`, `.next/`, `dist/`, `out/`, `.turbo/` — build output, excluded from the workspace and regenerated.
